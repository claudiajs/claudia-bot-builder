  ...
  api.post('/twilio', request => {
    ...
    let twilioHandle = parsedMessage => {
      ...
      return Promise.resolve(parsedMessage).then(parsedMessage => bot(parsedMessage, request))
        .then(botResponse => responder(request.env.TWILIO_ACCOUNT_SID, request.env.TWILIO_AUTH_TOKEN, request.env.TWILIO_NUMBER, parsedMessage.sender, botResponse))
        .catch(logError);
    };
          ...
          .then(results => {
            const deployment = {
              ...
              variables: {
                ...
                TWILIO_NUMBER: results['Twilio Number used for Sending']
              }
            };
