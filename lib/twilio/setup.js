  ...
  api.post('/twilio', request => {
    ...
    let twilioHandle = parsedMessage => {
      ...
      return Promise.resolve(parsedMessage).then(parsedMessage => bot(parsedMessage, request))
        .then(botResponse => responder(request.env.TWILIO_ACCOUNT_SID, request.env.TWILIO_AUTH_TOKEN, new Buffer(request.env.TWILIO_NUMBER).toString('ascii'), parsedMessage.sender, botResponse))
        .catch(logError);
    };
