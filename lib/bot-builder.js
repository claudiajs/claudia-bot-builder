...
module.exports = function botBuilder(messageHandler, options, optionalLogError) {
  ...
  if (!options || !options.platforms || options.platforms.facebook) {
    fbSetup(api, messageHandlerPromise, logError);
  }
  if (!options || !options.platforms || options.platforms.slackSlashCommand) {
    slackSetup(api, messageHandlerPromise, logError);
  }
  if (!options || !options.platforms || options.platforms.telegram) {
    telegramSetup(api, messageHandlerPromise, logError);
  }
  if (!options || !options.platforms || options.platforms.skype) {
    skypeSetup(api, messageHandlerPromise, logError);
  }
  if (!options || !options.platforms || options.platforms.twilio) {
    twilioSetup(api, messageHandlerPromise, logError);
  }
  if (!options || !options.platforms || options.platforms.kik) {
    kikSetup(api, messageHandlerPromise, logError);
  }
  if (!options || !options.platforms || options.platforms.groupme) {
    groupmeSetup(api, messageHandlerPromise, logError);
  }
  if (!options || !options.platforms || options.platforms.viber) {
