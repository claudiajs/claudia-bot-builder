  ...
  api.addPostDeployStep('facebook', (options, lambdaDetails, utils) => {
    return utils.Promise.resolve().then(() => {
      return utils.apiGatewayPromise.getStagePromise({
        ...
        if (data.variables.facebookAccessToken && !data.variables.facebookAppSecret)
          return console.log(`\n${color.yellow}Deprecation warning:${color.reset} Your bot is not using facebook payload validation. Please re-run ${color.yellow}claudia update${color.reset} command with ${color.yellow}--configure-fb-bot${color.reset} flag to set the facebook app secret and enable payload validation. This will become mandatory in the next major version. See https://github.com/claudiajs/claudia-bot-builder/blob/master/docs/API.md#message-verification for more information.\n\n`);
      });
    })
