      ...
      return utils.apiGatewayPromise.getStagePromise({
        ...
        if (options['configure-fb-bot']) {
          ...
          .then(() => {
            ...
            return prompt(['Facebook access token', 'Facebook App Secret']);
          })
          .then(results => {
            console.log('\n');
        ...
        if (!data.variables || !data.variables.facebookAppSecret)
          return console.log(`\n${color.yellow}Warning:${color.reset} Please run --configure-fb-bot command again or manually add facebookAppSecret stage variable to API Gateway, as this will be required in Claudia Bot Builder v2.x.\n`);
      });
    })
