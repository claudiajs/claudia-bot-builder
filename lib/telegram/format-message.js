...
function addReplyKeyboard(keyboardArray, resizeKeyboard, oneTimeKeyboard) {
...
function addInlineKeyboard(keyboardArray) {
  if (!Array.isArray(keyboardArray))
    throw new Error('KeyboardArray needs to be valid array of arrays for addInlineKeyboard method');

  this.template.inline_keyboard = JSON.stringify(keyboardArray);
...
class Text {
  ...
  addReplyKeyboard(keyboardArray, resizeKeyboard, oneTimeKeyboard) {
    let replyKeyboard = addReplyKeyboard.bind(this);
    return replyKeyboard(keyboardArray, resizeKeyboard, oneTimeKeyboard);
  }

  addInlineKeyboard(keyboardArray) {
    let inlineKeyboard = addInlineKeyboard.bind(this);
    return inlineKeyboard(keyboardArray);
  }
...
class Location {
  constructor(latitude, longitude, disableNotification) {
...
class Photo {
  constructor(photo, caption, disableNotification) {
