...
module.exports = function kSetup(api, bot, logError, optionalParser, optionalResponder) {
  ...
  api.post('/kik', request => {
    let kikMessages = request.body;

    let parsedMessage = parser(kikMessages);
    if (!parsedMessage){
      return Promise.resolve('ok');
    }
    return Promise.resolve(parsedMessage).then(parsedMessage => bot(parsedMessage, request))
      .then(botResponse => responder(parsedMessage, botResponse, request.env.kikApiKey))
      .catch(logError);
  });
          ...
          return prompt(['Kik Bot Username', 'Kik Api Key'])
            .then(results => {
              const deployment = {
                ...
                variables: {
                  username: results['Kik Bot Username'],
                ...
                .then(() => {
                  ...
                  let options = {
                    headers: {
                      ...
                      'Authorization': `Basic ${new Buffer(deployment.variables.username + ':' + deployment.variables.kikApiKey).toString('base64')}`
                    },
                    body: JSON.stringify({
